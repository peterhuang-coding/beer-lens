...
从图中抽取每款酒：beerName(酒名)、brewery(酒厂)、style(风格)、abv、ibu、price(图中原币种金额, 数字)、currency(明确币种代码，无法确定填unknown；不换汇)、servingMode(draught/can/bottle/unknown，按图中规格填写)、serving(容量, 如330ml)
...
- menuIndex 必须抄录图片上实际印刷的酒单编号，跳号必须保留，不能按数组位置重新编号；看不到编号时填0。rawText同时保留清楚可见的编号。
...
function beerSignalExtractSchema() { return { type: "object", additionalProperties: false, required: ["sourceType","rawText","items","visualBeerDescription","uncertainties"], properties: { sourceType: { type: "string", enum: ["menu","tap_list","bottle","can","glass","venue","text","unknown"] }, rawText: { type: "string" }, visualBeerDescription: { type: "object", additionalProperties: false, required: ["color","clarity","foam","visiblePackagingDate","notes"], properties: { color:{type:"string"}, clarity:{type:"string"}, foam:{type:"string"}, visiblePackagingDate:{type:"string"}, notes:{type:"array",items:{type:"string"}} } }, uncertainties: { type: "array", items: { type: "string" } }, items: { type: "array", items: { type: "object", additionalProperties: false, required: ["menuIndex","rawText","beerName","brewery","style","abv","ibu","price","serving","currency","servingMode","packagingDate","confidence"], properties: { menuIndex:{type:"integer"}, rawText:{type:"string"}, beerName:{type:"string"}, brewery:{type:"string"}, style:{type:"string"}, abv:{type:"number"}, ibu:{type:["number","null"]}, price:{type:["number","null"]}, serving:{type:"string"}, currency:{type:"string"}, servingMode:{type:"string"}, packagingDate:{type:"string"}, confidence:{type:"number"} } } } } }; }
