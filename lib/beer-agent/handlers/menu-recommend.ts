...
function genericRecommendationQueries(text: string): string[] {
  ...
  // ── Fallback: broad generic requests without specific style keywords ──
  // When user says "推荐女士适合的啤酒" or "推荐新手友好的啤酒", no specific
  // style is mentioned. Return a diverse spread of popular styles so the
  // recommendation engine has candidates to score and rank (reg_343 fix).
  if (queries.length === 0) {
    queries.push("IPA", "Stout", "Lager", "Sour");
  }
...
/**
 * Normalize a beer name for deduplication: case-insensitive, whitespace-collapsed.
 */
...
/**
 * Score how much data a candidate has (higher = more complete).
 * Used to decide which duplicate to keep.
 */
function candidateDataScore(c: ScoredCandidate): number {
  let score = 0;
...
/** Source priority: untappd > ratebeer/sqlite > generic */
function sourceRank(source?: string): number {
  if (source === "untappd") return 3;
  if (source === "ratebeer" || source === "sqlite") return 2;
  return 1;
}

/**
 * Deduplicate candidates by normalized display name.
 *
 * When the same beer appears from multiple query variants (e.g., "IPA",
 * "West Coast IPA", "Hazy IPA" all resolve to "Punk IPA"), only the
 * candidate with the most complete data is kept, preferring untappd
 * sources over SQLite over generic.
 *
 * Candidate IDs are reassigned sequentially after dedup.
 */
function deduplicateCandidates(candidates: ScoredCandidate[]): ScoredCandidate[] {
  ...
  for (const c of candidates) {
    ...
    // Keep the candidate with more data
    const existingScore = candidateDataScore(existing);
    const currentScore = candidateDataScore(c);
    if (currentScore > existingScore) {
      seen.set(key, c);
    } else if (currentScore === existingScore) {
      // Tiebreaker: prefer untappd source
  ...
  // Reassign unique sequential candidateIds
 ...
 *   3. Deduplicating candidates that resolve to the same beer
 */
async function resolveBeerCandidates(queries: string[]): Promise<{ candidates: ScoredCandidate[]; lookupResults: BeerLookupResult[] }> {
  ...
  // Deduplicate: when multiple query variants resolve to the same beer
  const deduped = deduplicateCandidates(candidates);

  return { candidates: deduped, lookupResults: results };
}
