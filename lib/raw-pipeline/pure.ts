// Pure functions for raw data pipeline — no I/O, fully testable.
import { createHash } from "node:crypto";
import type { CrawlItem, RawLabels, RawQuestion, RawTask, ImageUrlCheck } from "./types";
...
export function createRawTaskId(item: CrawlItem): string {
...
export function buildRawQuestions(): RawQuestion[] {
  return [
    {
      id: "is_beer_label",
      type: "yesno",
...
export function crawlItemToRawTask(item: CrawlItem): RawTask {
...
export function filterLabeledTasks(tasks: RawTask[]): RawTask[] {
...
const ALLOWED_LABEL_KEYS: (keyof RawLabels)[] = [
  "beerName",
  "brand",
  "style",
  "abv",
...
const ALLOWED_STATUSES: RawTask["status"][] = [
  "pending",
  "labeled",
  "skipped",
  "exported",
...
const ALLOWED_IMAGE_QUALITIES: NonNullable<RawLabels["imageQuality"]>[] = [
  "good",
  "ok",
  "bad",
  "unusable",
...
const ALLOWED_CONFIDENCES: NonNullable<RawLabels["confidence"]>[] = [
  "high",
  "medium",
  "low",
];
...
export function validatePatchBody(
  body: unknown,
): { valid: true; labels: RawLabels; status?: RawTask["status"] } | { valid: false; errors: PatchValidationError[] } {
  ...
  if ("status" in raw && raw.status !== undefined) {
    if (typeof raw.status !== "string" || !ALLOWED_STATUSES.includes(raw.status as RawTask["status"])) {
      errors.push({
        field: "status",
  ...
  if ("labels" in raw && raw.labels !== undefined) {
    if (typeof raw.labels !== "object" || raw.labels === null) {
      ...
      for (const [key, value] of Object.entries(labelObj)) {
        if (!ALLOWED_LABEL_KEYS.includes(key as keyof RawLabels)) {
...
export function labeledTaskToRegressionCase(task: RawTask): Record<string, unknown> | null {
