...
export function shouldUsePlanner(
  ...
  // Text-only menu_recommend with secondary intents (profile_query/unclear):
  // the handler already reads profile and has robust fallback logic.
  // Planner has no vision tool and its search_beer_db lacks fallback queries,
  // so routing through planner loses candidates (reg_343).
  if (isMultiIntent && intent === "menu_recommend" && !hasImage) {
    return {
      usePlanner: false,
      reason: `menu_recommend text-only — skip planner, handler has fallback`,
    };
  }
  if (isMultiIntent) {
    return {
