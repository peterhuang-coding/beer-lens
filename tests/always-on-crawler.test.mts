...
describe("dedupeAndMerge", () => {
  it("preserves priority order: gaps → user → seeds", () => {
    ...
    const user = [
      { name: "Custom IPA", brewery: "Some Brewery", priority: "p1_user" },
    ];
    const seeds = [
      { name: "Flying Fist", brewery: "Jing-A", priority: "p2_seed" },
    ];
  ...
  it("returns [] for empty inputs", () => {
    assert.deepStrictEqual(
      dedupeAndMerge({ gaps: [], user: [], seeds: [] }, 30),
      [],
    );
  });
  ...
  it("treats limit=0 as zero-cap (returns [])", () => {
    ...
    assert.deepStrictEqual(
      dedupeAndMerge({ gaps: items, user: [], seeds: [] }, 0),
      [],
    );
  });
});
...
it("deduplicates a brewery with a Chinese translation suffix, preserving priority", () => {
  const out = dedupeAndMerge({
    gaps: [{ name: "Hop Roulade", brewery: "YE Brewing", priority: "p0_gap" }],
    seeds: [{ name: "Hop Roulade", brewery: "YE Brewing （野鹅微醺）" }],
  });
  assert.equal(out.length, 1);
  assert.equal(out[0].priority, "p0_gap");
});
it("keeps distinct brewery branches in parentheses", () => {
  assert.equal(
    dedupeAndMerge({
      seeds: [
        { name: "IPA", brewery: "Example (North)" },
        { name: "IPA", brewery: "Example (South)" },
      ],
    }).length,
    2,
  );
