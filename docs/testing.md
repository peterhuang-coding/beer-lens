...
本次新增的确定性测试覆盖：预算区间与放宽、IBU 数值硬约束、菜单内单位价／总价目标、价量与差价回复、序号追问不覆盖完整菜单，以及便携 Skill 在仓库外工作目录运行。2026-09-14 本地完整代码测试为 **574 / 574 通过**，聊天输入 GUI 验收为 **3 / 3 通过**，类型检查与生产构建通过。真实图片模型与真实用户任务仍需单独复测，不能用这些结果推断视觉准确率或市场需求成立。
